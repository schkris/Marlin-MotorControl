...
void setup() {
  ...
  SET_INPUT(BTN_EN1);
  SET_INPUT(BTN_EN2);
  SET_INPUT(BTN_ENC); // If you're using BTN_ENC
...
const unsigned long MAX_STEP_DELAY = 2000;  // Maximum delay between steps (slowest speed)
const unsigned long MIN_STEP_DELAY = 10;    // Minimum delay between steps (fastest speed)
const unsigned long STEP_PULSE_DURATION = 5; // Duration of step pulse in microseconds
int encoderState = 0;
...
void controlStepperMotor(int speed) {
  ...
  digitalWrite(Z_DIR_PIN, (speed >= 0) ? HIGH : LOW); // Adjust as necessary
...
void loop() 
{
  do 
  {
    ...
    if (millis() - lastChangeTime > 2000) { // Change speed every 2 seconds
      stepperSpeed += 10;
      if (stepperSpeed > 255) stepperSpeed = 0;
